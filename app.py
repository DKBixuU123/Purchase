...
if st.session_state['logged_in']:
    ...
    # PHASE 2: PURCHASER (WITH CLEAR FORM FIX)
    if user == "Purchaser":
        ...
        if pending_ids:
            ...
            # --- FORM START ---
            # 'clear_on_submit=True' ensures fields go blank after clicking Save
            with st.form("quote_entry_form", clear_on_submit=True):
                v1, v2, v3 = st.columns(3)
                with v1: v_name = st.text_input("Supplier Name")
                with v2: v_price = st.number_input("Unit Price", min_value=0.0, step=1.0)
                with v3: v_disc = st.number_input("Discount %", min_value=0.0, step=0.1)
                ...
                if st.form_submit_button("Save Supplier Quote"):
                    ...
                    st.success(f"Quote for {v_name} saved! Form cleared for next supplier.")
                    # st.rerun() is not needed inside 'clear_on_submit' forms usually, 
                    # but we keep it to refresh the table below immediately.
                    st.rerun()
            ...
            st.info("No RFQs are currently pending.")
